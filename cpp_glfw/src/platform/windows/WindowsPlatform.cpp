...
namespace cpp_glfw
{
    ...
    void Platform::PlatformWaitEventsTimeout(double timeout)
    {
        if (timeout != timeout
            ...
            || timeout > DBL_MAX)
        {
            CPP_GLFW_ERROR("Invalid time %f", timeout);
            return;
        }
    ...
    const char* Platform::PlatformGetClipboardString()
    {
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to open clipboard!");
            return nullptr;
        }
        ...
        if (!object)
        {
            CPP_GLFW_ERROR_WIN32("Failed to convert clipboard to string!");
            CloseClipboard();
            return nullptr;
        }
        ...
        if (!buffer)
        {
            CPP_GLFW_ERROR_WIN32("Failed to lock global handle!");
            CloseClipboard();
            return nullptr;
        }
    ...
    void Platform::PlatformSetClipboardString(const char* string)
    {
        ...
        if (!object)
        {
            CPP_GLFW_ERROR_WIN32("Failed to allocate global handle for clipboard!");
            return;
        }
    ...
    const char* Platform::PlatformGetScancodeName(int32_t scancode)
    {
        ...
        {
            CPP_GLFW_ERROR("Invalid scancode!");
            return nullptr;
        }
    ...
    bool WindowsPlatform::LoadLibraries()
    {
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to load winmm.dll!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to load user32.dll!");
            return false;
        }
    ...
    bool WindowsPlatform::RegisterWindowClass()
    {
        ...
        wc.lpszClassName = CPP_GLFW_WINDOW_CLASS;

        //load user provided icon if available
        wc.hIcon = (HICON)LoadImageW(GetModuleHandleW(nullptr), CPP_GLFW_ICON, IMAGE_ICON, 0, 0, LR_DEFAULTSIZE | LR_SHARED);
        if (!wc.hIcon)
        {
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to register window class!");
            return false;
        }
    ...
    {
        UnregisterClassW(CPP_GLFW_WINDOW_CLASS, GetModuleHandleW(nullptr));
    }

    bool WindowsPlatform::CreateHelperWindow()
    {
        s_HelperWindowHandle = CreateWindowExW(WS_EX_OVERLAPPEDWINDOW,
            CPP_GLFW_WINDOW_CLASS,
            CPP_GLFW_HELPER_WINDOW_TITLE,
        ...
        {
            CPP_GLFW_TRACE("Failed to create helper window!");
            return false;
        }
    ...
    char* WindowsPlatform::WideStringToUTF8(const WCHAR* source)
    {
        ...
        if (!size)
        {
            CPP_GLFW_ERROR_WIN32("Failed to convert wide string to UTF8!");
            return nullptr;
        }
    ...
    WCHAR* WindowsPlatform::UTF8ToWideString(const char* source)
    {
        ...
        if (!count)
        {
            CPP_GLFW_ERROR_WIN32("Failed to convert UTF8 to wide string!");
            return nullptr;
        }

...
namespace cpp_glfw
{
    ...
    LRESULT CALLBACK WindowsWindow::WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        ...
        if (uMsg == WM_NCCREATE)
        {
            ...
            if (pCreate->lpCreateParams)
            {
                ...
                SetPropW(hwnd, CPP_GLFW_WINDOW_PROP, pThis);
        ...
        {
            pThis = (WindowsWindow*)GetPropW(hwnd, CPP_GLFW_WINDOW_PROP);
        }
            ...
            switch (uMsg)
            {
                ...
                case WM_DEVICECHANGE:
                {
                    ...
                    if (wParam == DBT_DEVICEARRIVAL)
                    {
                        ...
                        {
                            CPP_GLFW_TRACE("DetectJoystickConnection");
                    ...
                    else if (wParam == DBT_DEVICEREMOVECOMPLETE)
                    {
                        ...
                        if (dbh && dbh->dbch_devicetype == DBT_DEVTYP_DEVICEINTERFACE)
                        {
                            CPP_GLFW_TRACE("DetectJoystickDisconnection");
    ...
    HICON WindowsWindow::CreateIcon(const Image* image, int32_t xHot, int32_t yHot, bool icon)
    {
        ...
        if (!color)
        {
            CPP_GLFW_ERROR_WIN32("Failed to create a RGBA bitmap!");
            return NULL;
        }
        ...
        if (!mask)
        {
            CPP_GLFW_ERROR_WIN32("Failed to create a mask bitmap!");
            return NULL;
        }
        ...
        if (!handle)
        {
            if (icon)
            {
                CPP_GLFW_ERROR_WIN32("Failed to create icon!");
            }
            else
            {
                CPP_GLFW_ERROR_WIN32("Failed to create cursor!");
            }
        }
        ...
        const WindowConfig* windowConfig, const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig, Monitor* monitor)
    {
        ...
        if (!wideTitle)
        {
            CPP_GLFW_ERROR("Failed to create window with invalid title '%s'!", title.c_str());
        ...
        if (!windowHandle)
        {
            CPP_GLFW_ERROR_WIN32("Failed to create window '%s'!", title.c_str());
    ...
    WindowsWindow::~WindowsWindow()
    {
        ...
        if (m_Handle)
        {
            RemovePropW(m_Handle, CPP_GLFW_WINDOW_PROP);
    ...
    void WindowsWindow::PlatformSetRawMouseMotion(bool enabled)
    {
        ...
        if (enabled)
        {
            ...
            {
                CPP_GLFW_ERROR_WIN32("Failed to register raw input device!");
            }
        }
        else
        {
            ...
            if (!RegisterRawInputDevices(&rid, 1, sizeof(rid)))
            {
                CPP_GLFW_ERROR_WIN32("Failed to remove raw input device!");
            }
        }
    }
    ...
    LRESULT WindowsWindow::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
    {
        switch (uMsg)
        {
            ...
            case WM_INPUT:
            {
                ...
                {
                    CPP_GLFW_ERROR_WIN32("Failed to retrieve raw input data!");
                    break;
                }

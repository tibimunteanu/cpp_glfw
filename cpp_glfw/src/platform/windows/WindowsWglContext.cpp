...
namespace cpp_glfw
{
    ...
    bool WindowsWglContext::Init()
    {
        ...
        if (!s_WGL.instance)
        {
            CPP_GLFW_ERROR_WIN32("Failed to load opengl32.dll!");
            return false;
        }
        ...
        if (dummyPFDID == 0)
        {
            CPP_GLFW_ERROR_WIN32("Failed to choose pixel format for dummy context!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to set pixel format for dummy context!");
            return false;
        }
        ...
        if (!dummyRC)
        {
            CPP_GLFW_ERROR_WIN32("Failed to create dummy rendering context!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to make dummy context current!");
    ...
    bool WindowsWglContext::CreateContext(WindowsWindow* window, const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig)
    {
        ...
        if (contextConfig->api == ContextAPI::OpenGL)
        {
            if (contextConfig->forward)
            {
                ...
                {
                    CPP_GLFW_ERROR("A forward compatible OpenGL context requested but WGL_ARB_create_context is unavailable!");
                    return false;
                }
            }
            ...
            if (contextConfig->profile != ContextProfile::Any)
            {
                ...
                {
                    CPP_GLFW_ERROR("OpenGL context profile requested but WGL_ARB_create_context_profile is unavailable!");
                    return false;
                }
            }
        }
        else
        {
            ...
            {
                CPP_GLFW_ERROR("OpenGL ES requested but WGL_ARB_create_context_es2_profile is unavailable!");
                return false;
            }
        }
        ...
        if (!dc)
        {
            CPP_GLFW_ERROR("Failed to retrieve DC for window!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to retrieve PFD for selected pixel format!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR_WIN32("Failed to set selected pixel format!");
            return false;
        }
        ...
        if (s_WGL.ARB_CreateContext)
        {
            ...
            if (!contextHandle)
            {
                ...
                if (error == (0xc0070000 | ERROR_INVALID_VERSION_ARB))
                {
                    if (contextConfig->api == ContextAPI::OpenGL)
                    {
                        CPP_GLFW_ERROR("Driver does not support OpenGL version %i.%i", contextConfig->major, contextConfig->minor);
                    }
                    else
                    {
                        CPP_GLFW_ERROR("Driver does not support OpenGL ES version %i.%i", contextConfig->major, contextConfig->minor);
                    }
                }
                else if (error == (0xc0070000 | ERROR_INVALID_PROFILE_ARB))
                {
                    CPP_GLFW_ERROR("Driver does not support the requested OpenGL profile");
                }
                else if (error == (0xc0070000 | ERROR_INCOMPATIBLE_DEVICE_CONTEXTS_ARB))
                {
                    CPP_GLFW_ERROR("The share context is not compatible with the requested context!");
                }
                else
                {
                    if (contextConfig->api == ContextAPI::OpenGL)
                    {
                        CPP_GLFW_ERROR("Failed to create OpenGL context!");
                    }
                    else
                    {
                        CPP_GLFW_ERROR("Failed to create OpenGL ES context!");
                    }
                }
        ...
        else
        {
            ...
            if (!contextHandle)
            {
                CPP_GLFW_ERROR_WIN32("Failed to create OpenGL context!");
                return false;
            }
            ...
            if (share)
            {
                ...
                {
                    CPP_GLFW_ERROR_WIN32("Failed to enable sharing with specified OpenGL context!");
                    return false;
                }
            }
    ...
    void Context::PlatformMakeContextCurrent(Window* window)
    {
        if (window)
        {
            ...
            {
                CPP_GLFW_ERROR_WIN32("Failed to make context current!");
        ...
        else
        {
            ...
            {
                CPP_GLFW_ERROR_WIN32("Failed to clear current context!");
            }
    ...
    int32_t WindowsWglContext::GetClosestPixelFormat(HDC dc, const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig)
    {
        ...
        if (s_WGL.ARB_PixelFormat)
        {
            ...
            {
                CPP_GLFW_ERROR_WIN32("OpenGL GetPixelFormatAttribivARB failed. Could not retrieve pixel format count!");
                return 0;
            }
        ...
        for (int32_t i = 0; i < nativeCount; i++)
        {
            ...
            if (s_WGL.ARB_PixelFormat)
            {
                ...
                int32_t values[64];
                CPP_GLFW_ASSERT(attribs.size() <= 64, "Cannot get more than 64 attribs!");

                if (!s_WGL.getPixelFormatAttribivARB(dc, pixelFormat, 0, attribs.size(), attribs.data(), values))
                {
                    CPP_GLFW_ERROR_WIN32("OpenGL GetPixelFormatAttribivARB failed. Could not retrieve pixel format attributes!");
                    return 0;
                }
            ...
            else
            {
                ...
                {
                    CPP_GLFW_ERROR_WIN32("Failed to describe pixel format!");
                    return 0;
                }
        ...
        {
            CPP_GLFW_ERROR("The driver does not appear to support OpenGL!");
            return 0;
        }
        ...
        if (!closest)
        {
            CPP_GLFW_ERROR("Failed to find a suitable pixel format!");
            return 0;
        }

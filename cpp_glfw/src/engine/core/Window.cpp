...
namespace cpp_glfw
{
                           ...
                           const WindowConfig* windowConfig, const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig, Monitor* monitor)
    {
        if (title.empty())
        {
            CPP_GLFW_ERROR("Window title cannot be empty!");
            return nullptr;
        }
        ...
        if (width <= 0
            || height <= 0)
        {
            CPP_GLFW_ERROR("Invalid window size %i x %i", width, height);
            return nullptr;
        }

        if (!contextConfig->IsValid())
        {
            CPP_GLFW_ERROR("Context config is not valid!");
            return nullptr;
        }
    ...
    int32_t Window::GetInputMode(InputMode mode)
    {
        ...
        CPP_GLFW_ERROR("Invalid input mode %d", (int32_t)mode);
        return false;
    }
    ...
    void Window::SetTitle(const std::string& title)
    {
        if (title.empty())
        {
            CPP_GLFW_ERROR("Invalid window title '%s'!", title.c_str());
            return;
        }
    ...
    void Window::SetIcon(const std::vector<Image*>& images)
    {
        if (!images.size())
        {
            CPP_GLFW_ERROR("Parameter 'images' cannot be empty!");
            return;
        }
    ...
    void Window::SetSizeLimits(int32_t minWidth, int32_t minHeight, int32_t maxWidth, int32_t maxHeight)
    {
            ...
            && minHeight != -1)
        {
            if (minWidth < 0
                || minHeight < 0)
            {
                CPP_GLFW_ERROR("Invalid window minimum size %i x %i!", minWidth, minHeight);
                return;
            }
        }
            ...
            && maxHeight != -1)
        {
            if (maxWidth < 0
                ...
                || maxHeight < minHeight)
            {
                CPP_GLFW_ERROR("Invalid window maximum size %i x %i!", maxWidth, maxHeight);
                return;
            }
        }
    ...
    void Window::SetAspectRatio(int32_t numerator, int32_t denominator)
    {
            ...
            && denominator != -1)
        {
            if (numerator <= 0
                || denominator <= 0)
            {
                CPP_GLFW_ERROR("Invalid window aspect ration %i:%i!", numerator, denominator);
                return;
            }
        }
    ...
    void Window::SetOpacity(float opacity)
    {
        if (opacity != opacity
            ...
            || opacity > 1.0f)
        {
            CPP_GLFW_ERROR("Invalid window opacity %f", opacity);
            return;
        }
    ...
    void Window::SetMonitor(Monitor* monitor, int32_t x, int32_t y, int32_t width, int32_t height, int32_t refreshRate)
    {
        ...
        if (refreshRate < 0
            && refreshRate != -1)
        {
            CPP_GLFW_ERROR("Invalid refresh rate %i!", refreshRate);
            return;
        }
    ...
    void Window::SetInputMode(InputMode mode, int32_t value)
    {
        if (mode == InputMode::Cursor)
        {
            ...
            {
                CPP_GLFW_ERROR("Invalid cursor mode 0x%08X", value);
                return;
            }
        ...
        else if (mode == InputMode::RawMouseMotion)
        {
            ...
            {
                CPP_GLFW_ERROR("Raw mouse motion is not supported on this system!");
                return;
            }
        ...
        {
            CPP_GLFW_ERROR("Invalid input mode 0x%08X", mode);
        }
    }
    ...
    void Window::SetCursorPosition(double x, double y)
    {
        if (x != x
            ...
            || y > DBL_MAX)
        {
            CPP_GLFW_ERROR("Invlid cursor position %f %f", x, y);
            return;
        }

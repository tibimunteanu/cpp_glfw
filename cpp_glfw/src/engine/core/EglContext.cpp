...
namespace cpp_glfw
{
    ...
    bool EglContext::Init()
    {
        ...
        if (!s_EGL.handle)
        {
            CPP_GLFW_ERROR("Could not load EGL library!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("Could not load required EGL entry points!");
            terminate();
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("Failed to get EGL display: %s", GetErrorString(s_EGL.getError()));
            terminate();
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("Failed to initialize EGL: %s", GetErrorString(s_EGL.getError()));
            terminate();
            return false;
        }
    ...
    bool EglContext::CreateContext(Window* window, const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig)
    {
        if (!s_EGL.display)
        {
            CPP_GLFW_ERROR("EGL API is not avaliable!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("Failed to find a suitable EGLConfig!");
            return false;
        }
        ...
        if (contextConfig->api == ContextAPI::OpenGLES)
        {
            ...
            {
                CPP_GLFW_ERROR("Failed to bind EGL API OpenGLES: %s", GetErrorString(s_EGL.getError()));
                return false;
            }
        }
        else
        {
            ...
            {
                CPP_GLFW_ERROR("Failed to bind EGL API OpenGL: %s", GetErrorString(s_EGL.getError()));
                return false;
            }
        }
        ...
        {
            CPP_GLFW_ERROR("Failed to create EGL context: %s", GetErrorString(s_EGL.getError()));
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("Failed to create EGL window surface: %s", GetErrorString(s_EGL.getError()));
            return false;
        }
        ...
        if (!s_EGL.KHR_GetAllProcAddresses)
        {
            ...
            if (!client)
            {
                CPP_GLFW_ERROR("Failed to load EGL client library!");
                return false;
            }
        }
    ...
    void EglContext::EGLMakeContextCurrent(Window* window)
    {
        if (window)
        {
            ...
            {
                CPP_GLFW_ERROR("Failed to make EGL context current: %s", GetErrorString(s_EGL.getError()));
                return;
            }
        }
        else
        {
            ...
            {
                CPP_GLFW_ERROR("Failed to clear EGL context: %s", GetErrorString(s_EGL.getError()));
                return;
            }
        }
    ...
    void EglContext::EGLSwapBuffers(Window* window)
    {
        ...
        if (window != current)
        {
            CPP_GLFW_ERROR("The EGL context must be current on the calling thread when swapping buffers!");
            return;
        }
    ...
    bool EglContext::GetClosestEGLConfig(const ContextConfig* contextConfig, const FramebufferConfig* framebufferConfig, EGLConfig* result)
    {
        ...
        if (!nativeCount)
        {
            CPP_GLFW_ERROR("No EGLConfigs found!");
            return false;
        }
        ...
        {
            CPP_GLFW_ERROR("The driver does not appear to support OpenGL through EGL!");
            return false;
        }
